...
{
    public MessageSnapshot(MessageHistory messageHistory, DateTimeOffset editTimestamp, int changeIndex,
        string currentContent, string? previousMessageContent) :
        this(messageHistory, editTimestamp, changeIndex, currentContent, previousMessageContent, false)
    {
    }

    private MessageSnapshot(MessageHistory messageHistory, DateTimeOffset editTimestamp, int changeIndex,
        string currentContent, string? previousMessageContent, bool isAboutHistoryUnavailability)
    {
    ...
    {
        return new MessageSnapshot(messageHistory, default, -1, loc.Get("MessageHistory.PreviousUnavailable"), null,
            true);
    }
    ...
    public IImmutableList<Diff>? GetEdits()
    {
        return ChangeIndex switch
        {
            ...
            _ => Diff.Compute(PreviousMessageContent ?? "", CurrentContent, true, true, CancellationToken.None)
                .MakeHumanReadable()
        };
    }
}