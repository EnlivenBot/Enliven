    ...
    public class LocalizationManager {
        ...
        static LocalizationManager() {
            ...
            try {
                ...
                _logger.Info("Loaded languages: {lang}.", string.Join(", ", Languages.Select(pair => $"{pair.Key} - {pair.Value.TranslationCompleteness}%")));
            }
            catch (Exception e) {
        ...
        private static LocalizationPack LoadLocalizationPack(string path) {
            ...
            static string ProcessLayouts(string text, LocalizationPack localizationPack) {
                ...
                do {
                    startText = text;
                    text = ProcessLayout(_locLayoutRegex, text, s => localizationPack.TryGetEntry(s[0], s[1], out var value) ? value : throw new ArgumentException($"Locale entry {s[0]}.{s[1]} does not exists"));
                    text = ProcessLayout(_emojiLayoutRegex, text, values => CommonEmojiStrings.Instance.GetEmoji(values[0]));
            ...
            static string ProcessLayout(Regex layoutRegex, string text, Func<List<string>, string> replace) {
                foreach (Match? match in layoutRegex.Matches(text)) {
                    text = layoutRegex.Replace(text, replace(match!.Groups.Values.Skip(1).Select(group => group.Value).ToList()), 1);
                }
                return text;
            }
        }
