...
public abstract class ChainBase {
    ...
    private protected readonly Dictionary<string, Action<IEntry>> PersistentOnEndActions = new();
    ...
    protected ChainBase(string? uid, ILocalizationProvider loc) {
        ...
        if (uid != null) {
            if (_runningChains.TryGetValue(uid, out var previousChain)) previousChain.OnEnd.Invoke(new EntryLocalized("ChainsCommon.ReasonStartedNew"));
    ...
    private static ConcurrentDictionary<string, ChainBase> _runningChains { get; set; } = new();

    private protected EnlivenEmbedBuilder MainBuilder { get; set; } = new();
