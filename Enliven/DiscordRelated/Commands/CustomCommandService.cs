...
public class CustomCommandService : CommandService, IService
{
    ...
    private new async Task AddModulesAsync(Assembly assembly, IServiceProvider services)
    {
        ...
        {
            if (definedType is { IsPublic: false, IsNestedPublic: false }
    ...
    public Task<(KeyValuePair<CommandMatch, ParseResult>?, IResult)> FindAsync(ICommandContext context, int argPos,
        IServiceProvider services,
        MultiMatchHandling multiMatchHandling = MultiMatchHandling.Exception)
    {
        return FindAsync(context, context.Message.Content.Substring(argPos), services, multiMatchHandling);
    }

    public async Task<(KeyValuePair<CommandMatch, ParseResult>?, IResult)> FindAsync(ICommandContext context,
        ...
        MultiMatchHandling multiMatchHandling = MultiMatchHandling.Exception)
    {
        ...
        if (!searchResult.IsSuccess) return (null, searchResult);
        ...
        return Aliases[command].Select(info => new EmbedFieldBuilder
        {
            ...
            Value = $"{loc.Get($"Help.{info.Summary}")}\n" +
                    ...
                    (info.Parameters.Count == 0
                        ? ""
                        : "\n" + string.Join("\n",
                            info.Parameters.Select(x =>
                                $"`{x.Name}` - {(string.IsNullOrWhiteSpace(x.Summary) ? "" : loc.Get("Help." + x.Summary))}")))
        });
    }
    ...
    private static string GetAliases(IEnumerable<string> aliases)
    {
        ...
        foreach (var alias in aliases) s.Append($" `/{alias}` ");
