...
public class CustomCommandService : CommandService, IService {
    ...
    public async Task OnPreDiscordStart() {
        ...
        foreach (var customTypeReader in _typeReaders) AddTypeReader(customTypeReader.GetTargetType(), customTypeReader);
    ...
    public async Task<(KeyValuePair<CommandMatch, ParseResult>?, IResult)> FindAsync(ICommandContext context, string input, IServiceProvider? services,
        ...
        foreach (var match in commands) preconditionResults[match] = await match.Command.CheckPreconditionsAsync(context, services).ConfigureAwait(false);
