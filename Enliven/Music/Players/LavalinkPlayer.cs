...
public class LavalinkPlayer : ILavalinkPlayer, ILavalinkPlayerListener {
    ...
    public ValueTask SeekAsync(TimeSpan position, SeekOrigin seekOrigin,
        CancellationToken cancellationToken = default) {
        EnsureNotDestroyed();
        cancellationToken.ThrowIfCancellationRequested();

        using var a = CreateActivity(nameof(SeekAsync),
            "LavalinkPlayer seeking to {SeekPosition} from {SeekOrigin} (currently at {CurrentPosition})",
    ...
    async ValueTask ILavalinkPlayerListener.NotifyTrackEndedAsync(LavalinkTrack track, TrackEndReason endReason,
        ...
        using var a = CreateActivity(nameof(NotifyTrackEndedAsync),
            "LavalinkPlayer track {TrackName} ended due to {EndReason}", track.Title, endReason);
    ...
    ValueTask ILavalinkPlayerListener.NotifyTrackExceptionAsync(LavalinkTrack track, TrackException exception,
        ...
        using var a = CreateActivity(nameof(NotifyTrackExceptionAsync),
            "LavalinkPlayer track {TrackName} got an {ExceptionSeverity} exception - {ExceptionMessage}",
    ...
    ValueTask ILavalinkPlayerListener.
        NotifyTrackStartedAsync(LavalinkTrack track, CancellationToken cancellationToken) {
        cancellationToken.ThrowIfCancellationRequested();
        ArgumentNullException.ThrowIfNull(track);

        using var a = CreateActivity(nameof(NotifyTrackStartedAsync), "LavalinkPlayer track {TrackName} started",
            track.Title);
        a?.AddTag("TrackUrl", track.Uri);
    ...
    ValueTask ILavalinkPlayerListener.NotifyTrackStuckAsync(LavalinkTrack track, TimeSpan threshold,
        ...
        using var a = CreateActivity(nameof(NotifyTrackStartedAsync), "LavalinkPlayer track {TrackName} stuck",
            track.Title);
        a?.AddTag("TrackUrl", track.Uri);
    ...
    protected Activity? CreateActivity(string method, [StructuredMessageTemplate] string message,
