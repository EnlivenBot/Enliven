...
namespace Bot.Commands
{
    public class GetLogByMessage : ModuleBase
    {
        ...
        public async Task PrintChanges(
            [Remainder]
            [Summary(
                "ID сообщения, чью историю нужно вывести. Так-же можно использовать пару `ID канала`-`ID сообщения`")]
            string id)
        {
            ...
            if (message != null)
                await Context.Channel.SendMessageAsync("", false,
                    message.BuildEmbed(Localization.GetLanguage(Context.Guild.Id, Context.Channel.Id)));
        }
    }
