...
namespace Bot.DiscordRelated.Commands {
    public class CustomCommandService : CommandService {
        ...
        public Task<(bool, KeyValuePair<CommandMatch, ParseResult>?, IResult?)> FindAsync(ICommandContext context, int argPos, IServiceProvider services, MultiMatchHandling multiMatchHandling = MultiMatchHandling.Exception)
            => FindAsync(context, context.Message.Content.Substring(argPos), services, multiMatchHandling);

        public async Task<(bool, KeyValuePair<CommandMatch, ParseResult>?, IResult?)> FindAsync(ICommandContext context, string input, IServiceProvider? services,
            ...
            {
                return (false, null, searchResult);
            }
            ...
            if (successfulPreconditions.Length == 0)
            {
                ...
                return (false, null, bestCandidate.Value);
            }
            ...
            if (successfulParses.Length == 0)
            {
                ...
                return (false, bestMatch, bestMatch.Value);
            }

            //If we get this far, at least one parse was successful. Execute the most likely overload.
            var chosenOverload = successfulParses[0];
            return (true, chosenOverload, null);
